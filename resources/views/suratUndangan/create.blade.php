...
<form class="main-form" action="{{route('suratUndangan.store') }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="mb-3">
    ...
    <div class="mb-3" id="tanggal_keg">
        <label for="" class="form-label">Tanggal Kegiatan</label>
        <input type="date" name="tanggal_keg" placeholder="Tanggal Kegiatan" class="form-control @error('tanggal_keg') is-invalid @enderror" value="{{ old('tanggal_keg') }}">
        @error('tanggal_keg')
        <span class="text-danger">{{ $message }}</span>
        @enderror
    </div>
    <div class="mb-3" id="waktu_keg">
        <label for="" class="form-label">Waktu</label>
        <input type="text" name="waktu_keg" placeholder="Waktu Kegiatan" class="form-control @error('waktu_keg') is-invalid @enderror" value="{{ old('waktu_keg') }}">
        @error('waktu_keg')
        <span class="text-danger">{{ $message }}</span>
        @enderror
    </div>
    <div class="mb-3" id="tempat_keg">
        <label for="" class="form-label">Tempat</label>
        <input type="text" name="tempat_keg" placeholder="Tempat Kegiatan" class="form-control @error('tempat_keg') is-invalid @enderror" value="{{ old('tempat_keg') }}">
        @error('tempat_keg')
        <span class="text-danger">{{ $message }}</span>
        @enderror
    </div>
    <div class="mb-3">
        ...
        <input type="hidden">
        <textarea name="acara" id="acara"></textarea>
    </div>
    <div class="mb-3">
    ...
    <a href="{{ route('suratUndangan.index') }}" class="btn btn-secondary">Batal</a>
</form>
...
<script>
    $('#kode_instansi').on('change', function(e) {
        ...
        $.ajax({
            url: "{{route('generateLetterNumberUndangan')}}",
            method: "GET",
            headers: {
